...
static int phy_expanded_read_atomic(struct phy_device *phydev, u32 reg_addr)
{
	...
	bus->write(bus, phydev->mdio.addr, MII_EXPMA, reg_addr);
	ret = bus->read(bus, phydev->mdio.addr, MII_EXPMD);
				     ...
				     u32 reg_addr, u16 val)
{
	...
	bus->write(bus, phydev->mdio.addr, MII_EXPMA, reg_addr);
	ret = bus->write(bus, phydev->mdio.addr, MII_EXPMD, val);
...
static int festa_phy_probe(struct phy_device *phydev)
{
	...
	if (priv->wol_irq > 0) {
		ret = devm_request_irq(&phydev->mdio.dev, priv->wol_irq,
